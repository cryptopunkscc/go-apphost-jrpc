...
func TestExecutor_Call(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "function with json positional arguments decode as struct",
			fields: fields{
				function: testFunc5,
			},
			args: args{
		...
		{
			name: "function with json positional optional arguments decode as struct",
			fields: fields{
				function: testFunc5,
			},
			args: args{
			...
			wantResult: []any{TestArgPos{I: 1, B: true, TestArg2: &TestArg2{S: "a"}}},
			wantErr:    assert.NoError,
		},
		{
